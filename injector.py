...
class Injector:
    ...
    def engine(self):
        ...
        self.logger.info("[%u] Ready to ingest %s:%s" % (os.getpid(), self.config['host'], self.config['port']))
        return conn
    ...
    def ingest_from(self, producer):
        ...
        rows = 0
        try:
            for row in producer.generate_row():
                rows += 1
                ...
                if rows == 100:  # TODO add batch_size into config
                    ...
                    self.logger.info("[%u] loaded %d rows in %.2f seconds, %.2f rows/sec" % (os.getpid(), rows, took, rows/took))
